...
```bash
$ helm install <helm-repo>/traefik --name my-release --namespace kube-system
```
...
$ helm install --name my-release --namespace kube-system \
  --set dashboard.enabled=true,dashboard.domain=traefik.example.com <helm-repo>/traefik
```
...
```bash
$ helm install --name my-release --namespace kube-system --values values.yaml <helm-repo>/traefik
```
